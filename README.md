...
**廠務檢驗台。** 把現場的製程紀錄（CSV）送進來，系統會指出哪些製程參數最影響品質、調整後預測會如何變化，以及目前這份模型是否已通過品保核准、可以作為現場依據。
...
ExplainerDashboard、Shapash 這類分析工具算得出結果，但很難直接進廠使用：沒有廠內帳號與權限、沒有模型核准流程、沒有操作稽核，畫面也不適合值班人員操作。
...
適用範圍：再製造、導電膠／電極、塗佈、烘烤、組裝後品質。品質目標可以是連續值（例如導電率），也可以是類別（OK／NG、返工／報廢、A／B／C）。
...
2. **模型** — 選擇自動判斷、迴歸、二元分類或多類分類。分類結果會顯示混淆矩陣，以及每一類的精確率。
3. **關鍵因子** — 卡片先說明應該優先控制哪一項參數，以及它大約佔多少影響力。
4. **因子依存** — 固定其他條件，只變動一項參數，觀察品質預測的走勢。
...
內建廠內角色：系統管理員、工程師、品保、唯讀。唯讀帳號只能查看結果，不能訓練模型或送出模擬。
...
如果這個工具對你的現場分析或品保流程有幫助，歡迎用街口支付支持後續開發。
